package cn.hyperrain.peppershop;

import cn.hyperrain.peppershop.command.PluginCommand;
import cn.hyperrain.peppershop.localization.ItemNameTranslationConfig;
import cn.hyperrain.peppershop.stroage.ShopsConfig;
import cn.hyperrain.peppershop.stroage.PluginConfig;
import cn.hyperrain.peppershop.listener.*;
import cn.hyperrain.peppershop.localization.Localization;
...
public class PepperShop extends PluginBase
{
	...
	public void onEnable() 
	{
		...
		{
			getLogger().warning(TextFormat.colorize(Quick.t(pm_cannot_work_with_quickshop)));
		...
		{
			getLogger().warning(TextFormat.colorize(Quick.t(pm_cannot_work_with_quickshopx)));
			getServer().getPluginManager().disablePlugin(this);
		...
		if(EasyAPIAPILoaded)
		{
			Quick.info(pm_discovered_easyapi);
			...
			if(LandPluginLoaded)
			{
				for(Map.Entry<ModuleInfo, IEasyAPIModule> module : EasyAPIModuleManager.INSTANCE.getAllModule().entrySet())
				{
					if(module.getKey().getModuleOwner().getName().equals("Land"))
					{
						...
						Quick.info(pm_linked_with_land);
						break;
					}
				}
		...
		{
			getLogger().info(TextFormat.colorize(localization.get(pm_linked_with_residence)));
		}
		...
		if(GACPluginLoaded)
		{
			...
			{
				getLogger().error(TextFormat.colorize(localization.get(pm_gac_warning_1)));
				server.getPluginManager().disablePlugin(this);
				return;
			}else{
				getLogger().warning(TextFormat.colorize(localization.get(pm_gac_warning_2)));
			}
		}
